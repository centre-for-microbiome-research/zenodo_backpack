...
'''Given that REST API does not provide a way to access DOI-to-version metadata, the job of ZenodoBackpack
is to download & verify specific DOI link; as well as create an uploadable backpack'''

class ZenodoBackpack:

    def download_and_extract(self, directory, DOI, progress_bar=False):
        ...
        if DOI is not None:
            ...
            recordID = self.__retrieve_record_ID(DOI)
            metadata, files = self.__retrieve_record_metadata(recordID)
            ...
            for f in files:
                ...
                for _ in range(3):
                    try:
                        self.__download_file(link, os.path.join(directory, filename), progress_bar)
                ...
                if self.__check_hash(filename, checksum):
        ...
        #unzip - use md5sums.txt file created from metadata to get files
        ...
        self.__verify(directory, metadata, DOI)



    def __verify(self, directory, metadata, check_version=True):
        ...
        for payload_file in contents.keys():
            ...
            if not self.__check_hash(filepath, contents[payload_file], metadata=False):
    ...
    def __retrieve_record_ID(self, DOI):
    ...
    def __retrieve_record_metadata(self, recordID):
    ...
    def __check_hash(self, filename, checksum, metadata=True):
    ...
    def __download_file(self, file_url, out_file, progress_bar=False):
    ...
    def __extract_all(self, archive, extract_path):
...
class ZenodoBackpackCreator:
    def create(self, input_directory, output_file, data_version, force=False):
        ...
        try:
            _, filenames = self.__scandir(input_directory)
        except Exception as e:
            logging.error(e)
            raise e
        ...
        contents = {str(file).replace(parent_dir, ""): self.__md5sum_file(file) for file in filenames}
